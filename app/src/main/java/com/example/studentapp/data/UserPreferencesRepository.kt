...
interface UserRepository {
    val isUserRegistered: Flow<Int>
    suspend fun saveUserPreferences(isUserRegistered: Int)
}
...
class UserPreferencesRepository(private val dataStore: DataStore<Preferences>) : UserRepository {
    ...
    override val isUserRegistered: Flow<Int> = dataStore.data
        .catch {
        ...
        .map { preferences -> preferences[IS_USER_REGISTERED] ?: -1 }

    private companion object {
        val IS_USER_REGISTERED = intPreferencesKey("is_user_registered")
        const val TAG = "UserPreferencesRepo"
    }

    // функция edit работает атомарно (хз, чё это значит)
    override suspend fun saveUserPreferences(isUserRegistered: Int) {
